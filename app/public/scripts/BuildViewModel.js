define(['ko', 'moment', 'countdown'], function(ko, moment, countdown) {
  var BuildViewModel = function(build) {
    ...
    this.time = ko.forcibleComputed(function() {
      return this.isRunning() ? 'started ' + moment(this.startedAt()).fromNow() : this.isQueued() ? 'not started yet' : 'finished ' + moment(this.finishedAt()).fromNow();
    }, this);

    this.duration = ko.forcibleComputed(function() {
      return this.isRunning() ? 'running for ' + countdown(this.startedAt()).toString() : this.isQueued() ? 'queued for ' + countdown(this.startedAt(), this.queuedAt()).toString() : 'ran for ' + countdown(this.startedAt(), this.finishedAt()).toString();
    }, this);
